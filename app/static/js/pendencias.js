$(document).ready(function () {
  ...
  $(document).on("click", ".pagar-pendencia-btn", function () {
    const pendenciaId = $(this).data("id"); // Obtém o ID da pendência

    // Confirmação antes de marcar como pago
    Swal.fire({
      title: "Confirmar Pagamento",
      text: "Tem certeza de que deseja marcar esta pendência como paga?",
      icon: "warning",
      ...
      if (result.isConfirmed) {
        // Fazer a requisição para marcar como pago
        fetch(`/pendencias/pagar/${pendenciaId}`, {
          method: "POST",
        })
          .then((res) => {
            if (!res.ok) {
              throw new Error("Erro ao marcar pendência como paga.");
            }
            return res.json();
          })
          .then((data) => {
            if (data.success) {
              Swal.fire(
                "Pago!",
                "A pendência foi marcada como paga.",
                "success"
              );
      ...
      .then((data) => {
        if (data.success && data.pendencias.length > 0) {
          ...
          pendencias.forEach((pendencia) => {
            ...
            const precoUnitario = parseFloat(pendencia.preco_unitario).toFixed(
              2
            ); // Preço unitário
            const subtotal = parseFloat(pendencia.subtotal).toFixed(2); // Subtotal
  ...
  function carregarClientesComPendencias() {
    fetch("/pendencias/clientes/pendentes") // Nova rota que retorna apenas clientes com pendências
      .then((res) => {
        if (!res.ok)
          throw new Error("Erro ao carregar clientes com pendências.");
        return res.json();
      })
      .then((data) => {
